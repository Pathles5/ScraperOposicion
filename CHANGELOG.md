...
- `monitor.js`: la estrategia de detección de cambios pasa de **híbrido HEAD-first + hash-fallback** a **hash-only**. Razón: las webs de la CM regeneran `Last-Modified` en cada respuesta aunque el contenido no cambie (CDN/balanceador/A-B testing), generando falsos positivos en la rama HEAD-first. Ahora siempre hace GET + SHA-256 del HTML normalizado. Funciones `fetchHead()` y constante `HEAD_TIMEOUT_MS` eliminadas (ya no se usan).
- `monitor.js`: el mensaje de Telegram ahora **solo lista los sitios que han cambiado**. Antes mostraba todos los sitios (cambiados + sin cambios) con su estado. Ahora el bucle itera `summary.filter(s => s.changed)`, omitiendo los `✓ sin cambios`. En modo debug, si no hay cambios, se añade una nota explícita `_(sin cambios en ningún sitio — modo debug)_`. Footer actualizado a `⚠️ N sitio/s cambiaron.` (singular/plural correcto).
