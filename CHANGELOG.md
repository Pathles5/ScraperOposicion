...
- `scripts/raspberry/install.sh`: refactor de `detect_node_bin` a `detect_tool` (genérico) para reutilizar la misma lógica robusta con `pnpm` y `npm`. **Auto-instala dependencias** (`pnpm install --production` o fallback a `npm install --omit=dev`) si `node_modules/` no existe en el destino. Idempotente: re-ejecuciones saltan este paso si `node_modules/` ya está. Corrige el bug donde `install.sh` excluía `node_modules/` del rsync pero no lo regeneraba, dejando `monitor.js` sin `axios`/`cheerio` (error `ERR_MODULE_NOT_FOUND`).
